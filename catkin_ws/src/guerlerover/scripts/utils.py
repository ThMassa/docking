...
#     """Recupere une image (carte) de la zone definie par extent

#     Args:
#         extent (list): domaine [lon_min, lat_min, lon_max, lat_max] de la zone consideree
...
def compute_view_rectangle(center, east_width, north_height):
    """Calcule les limites [lon_min, lat_min, lon_max, lat_max] en fonction du centre [lon,lat] et de l'etendue en metres sur les deux axes

    Args:
        center (list): Centre [lon,lat] de la zone voulue
        east_width (float): etendue en metres de par et d'autre du centre selon l'axe EST/OUEST
        north_height (float): etendue en metres de par et d'autre du centre selon l'axe NORD/SUD
...
def plot_map(map, longitude_bounds, latitude_bounds, title, nro_fig=None):
    ...
    gg.set_xlabel("longitude (deg)")
    gg.set_ylabel("latitude (deg)")
    gg.set_title(title)
    gg.grid(True)
    #plt.pause(0.01)
    return gg
...
if __name__ == "__main__":
    ...
    ## Recuperation de la carte de la zone 
    background_image = load_background_image(extent)

    ## Affichage de la carte
    fig = plot_map(background_image, [extent[0], extent[2]], [extent[1], extent[3]], "Guerledan")
