...
\begin{frame}[fragile]{Filtre de Kalman}
  \begin{minipage}{0.5\textwidth}
    ...
    \begin{itemize}
      ...
      \item Mesure $\textbf{y}=(x_{GPS}, y_{GPS}, \psi_{IMU})^T$
    \end{itemize}
  \end{minipage}
  \begin{minipage}{0.49\textwidth}
    \begin{figure}[H]
...
\begin{frame}[fragile]{Guidage par champ de potentiel artificiel}
  ...
  \caption{(a) À gauche le robot se trouve dans le demi-plan en face du robot (b) À droite le robot se trouve dans le demi-plan derrière le robot}
\end{figure}
...
\begin{frame}[fragile]{Algorithme}
  \begin{minipage}{0.5\textwidth}
    Régulation en vitesse fait via cmd\_vel. \\
    Une régulation en cap via un PI. \\ \\
    Instabilité au niveau de la ligne séparatrice, conditions de transition ont été adaptées
    \begin{eqnarray}
